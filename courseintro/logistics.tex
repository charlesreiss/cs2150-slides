...
\begin{frame}{honor-related policies}
    ...
    \item do \textbf{not} look at \myemph<4>{another\tikzmark{anotherStudent} student's code}
    ...
    \begin{tikzpicture}[overlay,remember picture]
        \begin{visibleenv}<4>
        \node[mycallout=anotherStudent,align=left] at ([yshift=-1cm,xshift=-4cm]pic cs:anotherStudent) {
            past student, present student, \ldots
        };
        \end{visibleenv}
    \end{tikzpicture}
\end{frame}
