...
\begin{frame}{almost complete formally}
\begin{itemize}
...
\item otherwise: almost complete if either
    ...
    \item left child is almost complete with height $h$ and right child is complete with height $h-1$
    \end{itemize}
\end{itemize}
\begin{tikzpicture}
\tikzset{
    >=Latex,
    mybst/.style={binary tree layout,level distance=5mm,sibling distance=5mm,nodes={draw,circle,inner sep=0.5mm,minimum width=.5cm,text=white,font=\tiny}},
    missing/.style={alt=<1>{invisible},alt=<2>{dotted,draw=red},text=white, edge from parent/.style={draw=none}}
}
\begin{scope}[mybst]
\graph {
