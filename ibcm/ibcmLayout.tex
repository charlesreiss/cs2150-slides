...
\draw[instrBar](b-11 |- i-\a) rectangle (d-0 |- i-\b);
    ...
    \begin{visibleenv}<2>
        \node[anchor=north,red!70!black,align=center] at (b-13 |- i-4) {
            \textbf{opcode} \\
            which instruction?
        };
    \end{visibleenv}
    ...
    \begin{visibleenv}<6>
        ...
        \node[anchor=west,fill=white,draw=red,ultra thick,align=left] (ioOps) at ([xshift=.5cm]b-9 |- t-1) {
            \begin{tabular}{l|l}
                I/O op & effect \\ \hline \hline
                {\tt 00} & read hex word\\ \hline
                {\tt 01} & read ASCII character \\
                ~  & \small (into accumulator bits {\tt 0-7}) \\ \hline
                {\tt 10} & write hex word \\ \hline
                {\tt 11} & write ASCII charcater \\
