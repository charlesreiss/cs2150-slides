...
\node[right=1mm of stack.north east,program memory label,yshift=2mm] {0x7F\ldots{}};
...
\programMemoryImage
    \begin{visibleenv}<2>
        ...
        \node[right=0cm of heap,align=left] {grows upward \\ $\leftarrow$ new, malloc};
    \end{visibleenv}
    \begin{visibleenv}<3>
        \programMemoryHighlight{fit=(stack)}
        \node[right=0cm of stack,align=left,yshift=-2mm] {$\leftarrow$ local variables allocated here \\ grows downward};
    \end{visibleenv}
\end{tikzpicture}
\end{frame}
