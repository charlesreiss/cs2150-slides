...
\end{visibleenv}
\begin{visibleenv}<9>
\matrix[stack] (shuntStack) at (anchor) {
    \tt ( \\
    \tt / \\
    \tt + \\
};
\node[anchor=north west,inner sep=.25mm] (expr2) at (shuntStack.south west) {
    \tt + E) + F \strut
};
\node[anchor=north east,text=blue!70!black,inner sep=.25mm] (expr1) at (expr2.north west) {
    \tt A \myemph{+} B * C \myemph{/} \myemph{(}D  \strut
};
...
\begin{frame}{parenthsis matching}
\begin{tikzpicture}
\tikzset{
    >=Latex,
    point/.style={draw,very thick,<-},
...
\matrix[stack] (parenStack) {
    ...
    \tt \only<3->{\textbf<10>{\myemph<3>{[}}} \\
    \tt \only<2->{\myemph<2>{\{}} \\
};
\node[anchor=north east,inner sep=.25mm] (expr2) at ([yshift=-1cm]parenStack.south east) {
    \tt \myemph<2->{\{} \myemph<3->{[} \myemph<4->{(} \myemph<5->{)} \myemph<6->{[} \myemph<7->{]} \myemph<8->{(} \myemph<9->{)} \textbf<10->{\color{green!50!black}\}}
};
...
\matrix[stack] (stack) {
    \only<5-6>{8} \\
    \only<5-6>{7}\only<7-8>{$\frac{7}{8}$} \\
    \only<2-3>{7}\only<5-8>{4}\only<9-10>{$\frac{7}{2}$} \\
    \only<2-3>{3}\only<4->{21}\only<11>{$\frac{49}{2}$} \\
};
\node[draw,anchor=north east,inner sep=.25mm] (exprInfix) at ([yshift=-2cm]stack.south east) {
    \tt (3 * 7) + (4 * (7 / 8))
};
\node[draw,anchor=north east,inner sep=.25mm] (exprPostfix) at ([yshift=-1cm]stack.south east) {
