...
\begin{visibleenv}<3>
\node[box] {
    compile error (unless \lstinline|IntCell::operator<| created)! \\
...
\end{lstlisting} \\
compiler \myemph{needs implementation available} \\
~ \\
\myemph{required} to have imlpementation included in each .cpp file
};
\end{visibleenv}
\end{tikzpicture}
\end{frame}
